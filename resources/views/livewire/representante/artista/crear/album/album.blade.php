            ...
            <div class="flex">
                @foreach ($albumes as $index => $album)
                    <div class="flex flex-col items-center mr-5">
                        ...
                        <span>{{ $album['nombreAlbum'] }}</span>
                    </div>
                @endforeach
            </div>
            <div x-data="formNuevoAlbum()" 
                ...
                <div x-show.transition.out="estaAbierto()" @click.away='cerrar()'
                    class="bg-white absolute -top-40 left-40 p-4 text-primary w-96 border-4 shadow-md">
                    ...
                    <div class="mb-5 flex flex-col">
                        ...
                        <input type="text" wire:model="fechaLanz" class="mt-1" required />
                        @error('fechalanz')
                            <span class="text-red-600">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="mb-5 flex flex-col">
                        <label for="cantcan">¿Cuantas canciones tiene el album?</label>
                        <input type="number" wire:model="cantCan" class="mt-1" required />
                        @error('cantcan')
                            <span class="text-red-600">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="mb-5 flex flex-col">
                        ...
                        <div class="flex flex-col lg:mr-10">
                            <div class="flex mt-1">
                                <input class="bg-primary text-white placeholder-gray-300 border-0 p-0 w-36 mr-2" type="text" wire:model='nombreCancion' 
